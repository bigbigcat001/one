...
namespace Console_try
{
    ...
    internal class Program
    {
        ...
        static void ReceiveCallback(IAsyncResult ar)
        {
            try
            {
                ...
                else
                {
                    ...
                    string temSendInfo1 = Login(tempReadString);
        ...
        static string Login(string tempReadString)
        {
            string[] strings = tempReadString.Split(',');
            if (strings[0] == "1")
            {
                return MySqlHelper.Login_in_Click(strings[1], strings[2]);
            }
           else if (strings[0] == "2")
            {
                return MySqlHelper.Login_up_Click(strings[1], strings[2]);
